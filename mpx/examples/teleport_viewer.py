...
Supports:
  A) Single rollout file:
     python teleport_viewer_go2_rollout_npy.py --npy path/to/X.npy

  B) Sequence of rollout files via glob (e.g. X_00000.npy, X_00001.npy, ...):
     python teleport_viewer_go2_rollout_npy.py --npy "path/to/X_*.npy" --first-of-each

In "first-of-each" mode:
  - Load each matching file
  - Extract X[0] from each
  - Stack into a new trajectory X_seq of shape (K, nx)
  - Play X_seq in the viewer

Notes:
  - If the file contains a dict/object saved with allow_pickle=True,
    pass --allow-pickle and --key X (or your key).
"""
...
def load_first_states_from_sequence(
    pattern: str,
    *,
    key: str | None,
    allow_pickle: bool,
    ...
    """
    Load X_*.npy files matching `pattern`, extract X[0] from each, and stack.
    ...
    xs: List[np.ndarray] = []
    nx_ref: int | None = None

    for p in paths:
        X = load_X_from_npy(p, key=key, allow_pickle=allow_pickle)
        if X.ndim != 2:
            raise ValueError(f"{p}: expected X to be 2D (T, nx), got shape {X.shape}")
        if X.shape[0] < 1:
            raise ValueError(f"{p}: X has zero length in time dimension (T=0).")

        if nx_ref is None:
            nx_ref = int(X.shape[1])
        else:
            if int(X.shape[1]) != nx_ref:
                raise ValueError(
                    f"{p}: inconsistent state dimension nx={X.shape[1]} "
                    f"(expected nx={nx_ref})."
                )

        xs.append(np.asarray(X[0], dtype=np.float64))
...
def run_viewer(
    ...
    *,
    ...
    # Load X: either single file or a stacked sequence of X[0] from multiple files.
    if first_of_each:
        X = load_first_states_from_sequence(npy_path, key=key, allow_pickle=allow_pickle)
        print(f"[first-of-each] Loaded {X.shape[0]} files -> X_seq shape {X.shape}")
    else:
        assert os.path.exists(npy_path), f"Missing NPY: {npy_path}"
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            if realtime:
                # If you want this to reflect dt*stride, replace 0.1 with dt*stride
                time.sleep(0.1)
...
def parse_args():
    ...
    p.add_argument(
        ...
        help=(
            "Path to .npy containing X, or a pickled dict containing X (use --key). "
            "If using --first-of-each, this should be a glob pattern like 'mpc_data/X_*.npy'."
        ),
    )
    p.add_argument(
        "--first-of-each",
        action="store_true",
        help="Treat --npy as a glob pattern, load each file, take X[0] from each, and play the stacked trajectory.",
    )
    ...
    p.add_argument("--realtime", action="store_true", help="Sleep each frame.")
...
def main():
    ...
    run_viewer(
        ...
        first_of_each=bool(args.first_of_each),
    )
